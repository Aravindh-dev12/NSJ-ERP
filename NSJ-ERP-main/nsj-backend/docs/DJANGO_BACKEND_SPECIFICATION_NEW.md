...
#### 8. Archive Query (Admin Action)
**POST** `/api/payments/queries/{id}/archive/`
...
- Only allow archiving if status is 'pending' (or auto-expired queries)
- Update status to 'archived'
- Return updated query object
- Use case: Admin manually closes query if client loses interest, or query auto-archives when expired

---

#### 9. Reopen Query (Admin Action)
**POST** `/api/payments/queries/{id}/reopen/`
...
**Backend Logic:**
- Validate query exists and status is 'archived'
- Update status back to 'pending'
- Option: Extend expiry_date by default (e.g., +15 days from reopen date)
- Return updated query object
- Use case: Client renews interest in an expired query, admin reopens it

---

#### 10. Add Follow-Up Notes
**POST** `/api/payments/queries/{id}/add-follow-up/`

**Request Body:**
```json
{
    "follow_up_note": "Customer interested in matte finish variant. Will call back on 12th.",
...
```json
{
    ...
    "follow_up_notes": [
        {
            "id": "note-uuid",
            "note": "Customer interested in matte finish variant. Will call back on 12th.",
            "follow_up_date": "2025-12-12",
            "added_by": "admin_user",
            "added_at": "2025-12-03T14:30:00Z"
        }
    ],
    "last_follow_up": "2025-12-03T14:30:00Z",
...
**Backend Logic:**
- Validate query exists
- Create new FollowUpNote associated with query (or store as JSON array in query)
- Update query's last_follow_up timestamp
- Return query with updated follow_up_notes array
- Use case: Sales team tracks conversations and next steps with client

**Note:** Implement as either:
- **Option A:** Separate `FollowUpNote` model with ForeignKey to Query
- **Option B:** JSONField array stored in Query model

---

#### 11. Update Query Expiry (Admin Action)
**PATCH** `/api/payments/queries/{id}/`

Already covered in endpoint #4. Admins can update `expiry_date` to extend or reduce expiry.
...
#### 12. Bulk Auto-Archive Expired Queries (Scheduled Task)
**POST** `/api/payments/queries/auto-archive/`

Already covered in endpoint #6. Schedule this via Celery/APScheduler daily.

---

## Query Model - Additional Fields for Follow-Up Tracking

Update the Query model to support follow-up tracking:
...
class Query(models.Model):
    ...
    follow_up_notes = models.JSONField(default=list, blank=True)  # Array of follow-up entries
    last_follow_up = models.DateTimeField(blank=True, null=True)  # Last follow-up timestamp
    
    # Receipt voucher reference (for conversion tracking)
    receipt_voucher_id = models.CharField(max_length=255, blank=True, null=True)
    converted_order_id = models.UUIDField(blank=True, null=True)  # Reference to created order
    ...
    def __str__(self):
        return f"Query: {self.account.account_name if self.account else 'Unknown'}"
...
## Follow-Up Note Structure (for JSONField)

```python
# Each entry in follow_up_notes array
{
    "id": "uuid",
    "note": "Customer called - interested in 22K variant",
    "follow_up_date": "2025-12-12",  # Scheduled follow-up date
...
## Query Workflow States & Transitions

```
PENDING
    ↓ (if expiry_date passed)
ARCHIVED
    ↓ (if client renews interest)
PENDING (reopened)

PENDING
    ↓ (if payment received + receipt voucher linked)
CONVERTED_TO_ORDER → Order moves to Active Orders workflow
...
## Query List Response - Updated with Actions

When listing queries, include metadata for UI to show actions:
...
```json
{
    ...
    "results": [
        {
            "id": "uuid",
            "account": {"id": "uuid", "account_name": "John Doe"},
            "item_name": {"id": "uuid", "name": "Ring"},
            "gold_carat": "22K",
            "gender": "Woman",
            "size": "6 inches",
            "location": "Downtown",
            ...
            "actions": {
                "can_edit": true,
                "can_archive": true,
                "can_convert": false,  # Only if receipt voucher linked
                "can_reopen": false,
