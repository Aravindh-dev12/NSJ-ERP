...
export function JournalForm({ id }: JournalFormProps) {
  ...
  const [series, setSeries] = useState("JOURNAL M");
  ...
  useEffect(() => {
    if (id) {
      ...
      journalDetail(id)
        .then((data) => {
          setDate(data.date || "");
          setSeries(data.series || "JOURNAL M");
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      let result;
      if (id) {
        result = await journalUpdate(id, payload);
        toast({
          title: "Journal Updated",
          description: "Successfully updated the receipt voucher.",
        });
      } else {
        result = await journalCreate(payload);
        toast({
      ...
      if (result?.order_id && result?.id) {
        try {
          await autoCompleteStep(
            ...
            "Advance Received",
            result.id,
            `Journal ${id ? "updated" : "created"}: ${result.voucher_no || result.id}`
          );
        } catch (e) {
      ...
      if (shouldPrint && result?.id) {
        try {
          const data = await journalDetail(result.id);
          ...
          generateVoucherPDF("Journal", {
            voucherType: "Journal",
            date: data.date || "-",
            voucherNo: data.voucher_no || "-",
            series: data.series || "JOURNAL M",
            entries,
