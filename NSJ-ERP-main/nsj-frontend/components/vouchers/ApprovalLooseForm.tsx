...
export default function ApprovalLooseForm() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    void Promise.all([accountsDropdown(), vouchersItemNames()])
      .then(([accs, itemsResp]) => {
        if (!mounted) return;
        setAccounts(accs ?? []);
        // vouchersItemNames() returns an object { item_names: [...] }
    ...
    // fetch stamps and units via vouchersMasters and vouchersUnits helpers
    void (async () => {
      try {
        const [mastersResp, unitsResp] = await Promise.all([
          vouchersMasters(),
          vouchersUnits(),
        ]);
        if (mounted) {
          setStamps(mastersResp?.stamps ?? []);
          // vouchersUnits returns { units: [...] }
  ...
  const handleSave = async () => {
    ...
    if (!itemId)
      return toast({
        variant: "destructive",
        title: "Please select an item name",
      });
    ...
    try {
      await approvalLooseCreate(payload);
      toast({ title: "Saved", description: "Approval Loose M saved" });
      // clear form
      setOrderNumber("");
      setRemark("");
      setPiece("");
