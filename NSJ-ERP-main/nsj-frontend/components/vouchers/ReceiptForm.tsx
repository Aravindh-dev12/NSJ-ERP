...
export function ReceiptForm({ id }: ReceiptFormProps) {
  ...
  const [series, setSeries] = useState("RECEIPT M");
  const [voucherNo, setVoucherNo] = useState("");

  // Credit Entry State (Fixed Row 1)
  const [crPartyId, setCrPartyId] = useState("");
  const [crSubAc, setCrSubAc] = useState("");
  const [crBalance, setCrBalance] = useState<number | null>(null);
  const [crBalanceType, setCrBalanceType] = useState("");
  const [crAmount, setCrAmount] = useState<number>(0);
  ...
  // Load Accounts
  useEffect(() => {
    // Load standard accounts for party side
    accountsDropdown()
    ...
    // Load cash/bank accounts for debit side
    accountsDropdown({ type: "contra" })
      .then((data) => setCashBankAccounts(data || []))
      .catch(() =>
        toast({
          variant: "destructive",
          title: "Failed to load cash/bank accounts",
        })
      );
  }, [toast]);
    ...
    if (!id) {
      receiptNextVoucherNo()
        .then((res: any) => {
          ...
          else if (res?.data?.voucher_no) setVoucherNo(res.data.voucher_no);
        })
        .catch((err) =>
  ...
  useEffect(() => {
    if (id) {
      ...
      receiptDetail(id)
        .then((data) => {
          setDate(data.date || "");
          setSeries(data.series || "RECEIPT M");
          setVoucherNo(data.voucher_no || "");
          setCrPartyId(data.party_name_id || data.party_name?.id || "");
          setCrAmount(parseFloat(data.cr) || 0);
          setCrBalance(parseFloat(data.balance) || null);
          let loadedCrNarration = data.cr_narration || "";
          ...
          if (loadedCommonNarration.includes(" |GEN_REMARKS| ")) {
            ...
            loadedCrNarration = parts[0];
            loadedCommonNarration = parts.slice(1).join(" |GEN_REMARKS| ");
          }

          setCommonNarration(loadedCommonNarration);
          setCrNarration(loadedCrNarration);
          setCrSubAc(data.sub_ac || data.sub_account || "");

          if (data.debit_entries && Array.isArray(data.debit_entries)) {
            setDebitEntries(
              data.debit_entries.map((d: any) => ({
                party_id: d.party_id || d.party?.id || d.party_name_id || "",
                sub_ac: d.sub_ac || d.sub_account || "",
                balance: parseFloat(d.balance) || null,
                balance_type: d.balance_type || "",
                dr: parseFloat(d.dr) || 0,
  ...
  // Handle Credit Party Balance & Subaccounts Fetch
  useEffect(() => {
    if (crPartyId) {
      if (!subAccountsMap[crPartyId]) fetchSubAccounts(crPartyId);

      accountBalance(crPartyId)
        .then((res) => {
          setCrBalance(parseFloat(String(res.balance)));
          setCrBalanceType(
            res.balance_type ||
  ...
  }, [crPartyId, subAccountsMap, fetchSubAccounts]);

  // Handle Debit Parties Subaccounts Fetch on load
  useEffect(() => {
  ...
  }, [debitEntries, subAccountsMap, fetchSubAccounts]);

  // Calculation Helpers
  const totalCr = useMemo(() => parseFloat(String(crAmount || 0)), [crAmount]);
  const totalDr = useMemo(() => {
    const sum = debitEntries.reduce(
      (acc, entry) => acc + (parseFloat(String(entry.dr)) || 0),
      0
    );
  ...
  const handleRemoveDrRow = (index: number) => {
    setDebitEntries((prev) => {
      if (prev.length > 1) {
  ...
  const isSaveDisabled =
    ...
    totalCr <= 0 ||
    !debitEntries.some(
      (d) => d.party_id && (parseFloat(String(d.dr)) || 0) > 0
    ) ||
    !isBalanced ||
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (crNarration) {
        finalNarration = `${crNarration} |GEN_REMARKS| ${commonNarration}`;
      }
      ...
      const payload = {
        ...
        party_name_id: crPartyId,
        cr: totalCr,
        sub_ac: crSubAc,
        sub_account: crSubAc,
        narration: finalNarration,
        cr_narration: crNarration,
        debit_entries: debitEntries
          .filter((d) => d.party_id && (parseFloat(String(d.dr)) || 0) > 0)
          .map((d) => ({
            party_id: d.party_id,
            dr: parseFloat(String(d.dr)),
            balance: d.balance,
            sub_ac: d.sub_ac,
            sub_account: d.sub_ac,
            narration: d.narration,
          })),
      };
      ...
      let result;
      if (id) {
        result = await receiptUpdate(id, payload);
        toast({
          title: "Receipt Updated",
          description: "Successfully updated the receipt voucher.",
        });
      } else {
        result = await receiptCreate(payload);
        toast({
          title: "Receipt Created",
          description: "Successfully saved the new receipt voucher.",
        });
      }
      ...
      if (result?.order_id && result?.id) {
        try {
          await autoCompleteStep(
            ...
            "Advance Received",
            result.id,
            `Receipt ${id ? "updated" : "created"}: ${result.voucher_no || result.id}`
          );
        } catch (e) {
      ...
      if (shouldPrint && result?.id) {
        try {
          const data = await receiptDetail(result.id);
  ...
  const cashBankOptions = useMemo(
    () => cashBankAccounts.map((p) => ({ value: p.id, label: p.name })),
    [cashBankAccounts]
  );

  if (loading)
    return (
      <div className="p-10 text-center font-bold text-rose-300">
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto pb-10 px-2 sm:px-4">
      <Card className="border-rose-200 shadow-xl bg-white overflow-visible">
                  ...
                  <tr className="bg-[#fffafa] h-10 group">
                    ...
                    <td className="border-r border-rose-100 p-0 relative overflow-visible">
                      <SearchableSelect
                        ...
                        triggerClassName="border-none bg-transparent shadow-none h-10 w-full text-[13px] focus:ring-0 px-4 hover:bg-rose-50/50"
                      />
                    </td>
                    <td className="border-r border-rose-100 p-0 h-10 relative">
                      {crPartyId &&
                      subAccountsMap[crPartyId]?.options?.length > 0 ? (
                        <select
                          value={crSubAc}
                          onChange={(e) => setCrSubAc(e.target.value)}
                          className={cn(
                            inputBaseClasses,
                        ...
                        >
                          ...
                          {subAccountsMap[crPartyId].options.map((opt: any) => (
                            <option
                              key={opt.id}
                        ...
                        <Input
                          ...
                          placeholder={
                            crPartyId && subAccountsMap[crPartyId]?.loading
                              ? "Loading..."
                              : ""
                          }
                          disabled={
                            crPartyId
                              ? !!subAccountsMap[crPartyId]?.loading
                              : false
                          }
                        />
                      )}
                    ...
                    <td className="border-r border-rose-100 p-0 h-10 bg-[#fafafa]/50">
                      ...
                      >
                        {renderBalance(crBalance, crBalanceType)}
                      </div>
                    </td>
                    <td className="border-r border-rose-100 p-0 h-10 bg-gray-50/30">
                      <Input
                        disabled
                        className="border-none bg-transparent h-10 w-full cursor-not-allowed"
                      />
                    </td>
                    <td className="border-r border-rose-100 p-0 h-10">
                      <Input
                        ...
                        value={crAmount || ""}
                        onChange={(e) =>
                          setCrAmount(parseFloat(e.target.value) || 0)
                        }
                        placeholder="0.00"
                        className={cn(
                          inputBaseClasses,
                    ...
                    >
                      ...
                      <td className="border-r border-rose-100 p-0 relative overflow-visible">
                        <SearchableSelect
                          ...
                          onChange={(val) =>
                            updateDebitEntry(idx, "party_id", val)
                          }
                      ...
                      <td className="border-r border-rose-100 p-0 h-10 relative">
                        ...
                        subAccountsMap[entry.party_id]?.options?.length > 0 ? (
                          <select
                            value={entry.sub_ac}
                            onChange={(e) =>
                              updateDebitEntry(idx, "sub_ac", e.target.value)
                            }
                            className={cn(
                              inputBaseClasses,
                              "border-transparent ring-0 focus:ring-rose-400 focus:border-transparent outline-none cursor-pointer"
                      ...
                      <td className="p-0 h-10">
                        <Input
                          ...
                          onChange={(e) =>
                            updateDebitEntry(idx, "narration", e.target.value)
                          }
                          className={inputBaseClasses}
                ...
                <div className="w-full lg:w-[30%] flex flex-col items-end space-y-8 pt-2">
                  <div className="text-right space-y-2 group">
                    <Label className="text-[11px] font-bold text-rose-400 uppercase tracking-wider block transition-colors group-hover:text-rose-600 text-nowrap">
                      Credit Party Final Balance
                    </Label>
                    <div className="text-3xl font-black text-rose-700 tabular-nums leading-none tracking-tighter">
                      {crBalance !== null
                        ? `${crBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 })} ${crBalanceType}`
                        : "₹0.00"}
                    </div>
                  </div>
      ...
      <div className="mt-8 mx-auto max-w-2xl bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex flex-wrap justify-center gap-x-8 gap-y-2 opacity-60 hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-2">
          <div
            className={cn(
              "w-2 h-2 rounded-full",
              crPartyId ? "bg-green-500" : "bg-rose-300"
            )}
          ></div>
          <span className="text-[10px] font-bold text-gray-500 uppercase">
            Cr Party: {crPartyId ? "OK" : "MISSING"}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div
            className={cn(
              "w-2 h-2 rounded-full",
              totalCr > 0 ? "bg-green-500" : "bg-rose-300"
            )}
          ></div>
          <span className="text-[10px] font-bold text-gray-500 uppercase">
            Amount: {totalCr > 0 ? "OK" : "ZERO"}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div
