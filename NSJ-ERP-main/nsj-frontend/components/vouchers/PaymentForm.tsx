...
export function PaymentForm({ id }: PaymentFormProps) {
  ...
  const [series, setSeries] = useState("PAYMENT M");
  ...
  useEffect(() => {
    ...
    // Load cash/bank accounts for credit side
  ...
  // Load Next Voucher Number on mount (only for new payments)
  useEffect(() => {
    if (!id) {
      paymentNextVoucherNo()
        .then((res) => {
  ...
  const isSaveDisabled =
    ...
    totalCr <= 0 ||
    !creditEntries.some(
      (d) => d.party_id && (parseFloat(String(d.cr)) || 0) > 0
    ) ||
    !isBalanced ||
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const validCrEntry = creditEntries.find(
        (d) => d.party_id && (parseFloat(String(d.cr)) || 0) > 0
      );
      if (!validCrEntry) {
        toast({
          variant: "destructive",
          title: "Missing Credit Entry",
          description: "At least one Credit entry is required.",
        });
        setSubmitting(false);
        return;
      }
      ...
      const payload = {
        ...
        credit_entries: creditEntries
          .filter((d) => d.party_id && (parseFloat(String(d.cr)) || 0) > 0)
          .map((d, index) => ({
            party_id: d.party_id,
            cr: parseFloat(String(d.cr)),
            balance: d.balance,
            sub_ac: d.sub_ac,
      ...
      let result;
      if (id) {
        result = await paymentUpdate(id, payload);
        toast({
          title: "Payment Updated",
          description: "Successfully updated the payment voucher.",
        });
      } else {
        result = await paymentCreate(payload);
        toast({
          title: "Payment Created",
          description: "Successfully saved the new payment voucher.",
        });
      }
      ...
      if (result?.order_id && result?.id) {
        try {
          await autoCompleteStep(
            ...
            "Advance Received",
            result.id,
            `Payment ${id ? "updated" : "created"}: ${result.voucher_no || result.id}`
          );
        } catch (e) {
      ...
      if (shouldPrint && result?.id) {
        try {
          const data = await paymentDetail(result.id);
                    ...
                    >
                      ...
                      <td className="border-r border-rose-100 p-0 relative overflow-visible">
                        <SearchableSelect
                          ...
                          onChange={(val) =>
                            updateCreditEntry(idx, "party_id", val)
                          }
