...
type SalesReturnValues = z.infer<typeof salesReturnSchema>;
...
export function SalesReturnForm() {
  ...
  } = useForm<SalesReturnValues>({
    resolver: zodResolver(salesReturnSchema) as any,
    defaultValues: {},
  });
  ...
  const onSubmit = async (values: SalesReturnValues) => {
    ...
    try {
      await salesReturnCreate(payload);
      toast({
        title: "Sales return created",
        description: "Saved successfully.",
      });
      router.push("/vouchers/sales-return/list");
      router.refresh();
    } catch (err) {
      toast({
        variant: "destructive",
        title: "Could not create Sales Return",
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader className="space-y-2">
          ...
          <CardDescription>Fill in sales return details below.</CardDescription>
        </CardHeader>
        <CardContent>
          <form
