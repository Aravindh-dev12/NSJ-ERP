...
import { ArrowLeft, Search, Package, Plus, Pencil, Trash2 } from "lucide-react";
...
export default function ItemNamesPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await itemNamesList();
      setItems(data);
      setFilteredItems(data);
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Failed to load item names",
  ...
  const submitCreate = async () => {
    ...
    try {
      await itemNameCreate({
        name: formData.name.trim(),
        code: formData.code.trim() || undefined,
      });
      toast({
        title: "Item name created successfully",
      });
      ...
      toast({
        variant: "destructive",
        title: "Failed to create item name",
        description: error instanceof Error ? error.message : "Unknown error",
      });
    } finally {
  ...
  const submitEdit = async () => {
    ...
    setIsSubmitting(true);
    try {
      await itemNameUpdate(selectedItem.id, {
        name: formData.name.trim(),
        code: formData.code.trim() || undefined,
      });
      toast({
      ...
      toast({
        variant: "destructive",
        title: "Failed to update item name",
        description: error instanceof Error ? error.message : "Unknown error",
      });
    } finally {
      setIsSubmitting(false);
  ...
  const submitDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await itemNameDelete(selectedItem.id);
      toast({
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Jewelry types and product categories
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Item Names ({filteredItems.length})</CardTitle>
          <CardDescription>
            {searchQuery
              ? `Showing ${filteredItems.length} of ${items.length} items`
              : "Complete list of jewelry types and product categories"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new jewelry type or product category
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4 px-6">
            <div className="space-y-2">
              ...
              <Input
                id="create-name"
                placeholder="e.g., RING, BANGLE, NECKLACE"
                value={formData.name}
                onChange={(e) =>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update the jewelry type or product category
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4 px-6">
            <div className="space-y-2">
