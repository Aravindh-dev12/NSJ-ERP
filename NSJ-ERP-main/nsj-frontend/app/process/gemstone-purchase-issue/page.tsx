...
import {
  gemstonePurchaseIssueCreate,
  gemstonePurchaseIssueList,
  ordersDropdown,
  type GemstonePurchaseIssue,
  autoCompleteStep,
  saveProcessStepData,
} from "@/lib/backend";
import {
...
export default function GemstonePurchaseIssuePage() {
  const router = useRouter();
  const { user } = useAuth();
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [records, setRecords] = useState<GemstonePurchaseIssue[]>([]);
  ...
  useEffect(() => {
    const loadRecords = async () => {
      try {
        const response = await gemstonePurchaseIssueList({
          page: 1,
          page_size: 10,
        });
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const result = await gemstonePurchaseIssueCreate(formData);
      ...
      if (orderId) {
        try {
          await saveProcessStepData(
            ...
            result?.id,
            `Gemstone Purchase/Issue saved: ${result?.id}`
          );
        } catch (saveErr) {
      ...
      setError(err?.message || "Failed to save Gemstone Purchase/Issue record");
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      if (orderId && savedRecordId) {
        await autoCompleteStep(
          ...
          "Gemstone Purchase/Issue",
          savedRecordId,
          `Gemstone Purchase/Issue completed: ${savedRecordId}`
        );
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {isProductionDept
              ? "Manage gemstone purchase/issue process with order validation"
              : "View gemstone purchase/issue records (Read-only)"}
          </p>
        </div>
      </div>
      ...
      {isProductionDept && (
        <Card>
          <CardHeader>
            <CardTitle>Add New Gemstone Purchase/Issue</CardTitle>
          </CardHeader>
          <CardContent>
            <form
              onSubmit={(e) => {
            ...
            >
              ...
              {success && (
                <div className="flex items-center gap-2 rounded-md bg-green-50 p-3 text-sm text-green-700">
                  <CheckCircle2 className="h-4 w-4" />
                  {isCompleted
                    ? "Gemstone Purchase/Issue step marked as done!"
                    : "Gemstone Purchase/Issue record saved successfully!"}
                </div>
              )}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Recent Gemstone Purchase/Issue Records</CardTitle>
        </CardHeader>
        <CardContent>
          {loadingList ? (
