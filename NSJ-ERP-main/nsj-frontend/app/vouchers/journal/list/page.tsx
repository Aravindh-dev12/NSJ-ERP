...
import { journalsList, journalDelete, journalDetail } from "@/lib/backend";
...
export default function JournalListPage() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        toast({ variant: "destructive", title: "Unable to load journals" });
      } finally {
  ...
  const handlePrint = async (id: string) => {
    ...
    try {
      ...
      // Add main debit entry (primary debit party)
      if (data.party_name_id || data.party_name) {
        let partyName = "-";
        // Check for 'party' object first
        if (data.party && typeof data.party === "object") {
          partyName = data.party.account_name || data.party.name || "-";
        }
        // Fallback to 'party_name' field
        else if (
          typeof data.party_name === "object" &&
      ...
      const totalCredit =
        data.credit_entries?.reduce(
          (sum: number, c: any) => sum + parseFloat(String(c.cr || 0)),
          0
        ) || 0;
      ...
      <Card>
        <CardHeader className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">Journal list</CardTitle>
              <CardDescription>
                Search and maintain journal records.
              </CardDescription>
            </div>
            <div>
              <Link
                href="/vouchers/journal/new"
                  ...
                  {records.map((rec) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 font-bold text-emerald-900 border-r border-emerald-50/50 uppercase tabular-nums">
                          {rec.voucher_no ||
                            rec.voucher_number ||
                            rec.v_no ||
                            "—"}
                        </td>
                        <td className="px-4 py-4 border-r border-emerald-50/50">
                        ...
                        <td className="flex items-center justify-end gap-2 px-4 py-4 text-sm">
                          ...
                          <Link href={`/vouchers/journal/detail/${rec.id}`}>
                            <Button
                              variant="outline"
                              size="sm"
                              className="mr-2 h-8"
