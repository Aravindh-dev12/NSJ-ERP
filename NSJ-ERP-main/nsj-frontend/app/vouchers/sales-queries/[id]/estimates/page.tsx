...
export default function SalesQueryEstimatesPage() {
  ...
  // Backend now returns only estimates linked to this specific sales query
  const filteredEstimates = all_estimates || [];
        ...
        {can_create_variation && (
          <Button
            onClick={() => {
              ...
              const goldQuality =
                salesQuery?.gold_quality || summary?.gold_quality || "";
              const sizeDetails =
                salesQuery?.size_details || summary?.size_details || "";
              const salesPersonName =
                salesQuery?.sales_person || summary?.sales_person || "";
              ...
              if (goldQuality) {
                url += `&gold_quality=${encodeURIComponent(goldQuality)}`;
              }
              if (sizeDetails) {
                url += `&size_details=${encodeURIComponent(sizeDetails)}`;
              }
              if (salesPersonName) {
                url += `&sales_person_name=${encodeURIComponent(salesPersonName)}`;
              }
