...
import { contraOverview } from "@/lib/backend";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";

export default function ContraOverviewPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const resp = await contraOverview();
  ...
  return (
    <div className="space-y-8">
      <PreviousBackButton />
      <SalesHeader
        title="Contra overview"
        description="Monitor contra entries and create new contras."
        links={[
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 border-rose-100 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg">Recent contras</CardTitle>
            <CardDescription>
              The last 5 contra vouchers created.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
